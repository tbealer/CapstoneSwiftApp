...
//        addChild(wallIndicator)
        ...
        let lvlindicator = SKSpriteNode(imageNamed: "thereallvl6btn")
    ...
    func loadLevel() {
        if let levelPath = NSBundle.mainBundle().pathForResource("level6", ofType: "txt") {
        ...
        if node.name == "shark" {
            ...
            if (bomb2 >= 1) {
                ...
                player.runAction(sequence) { [unowned self] in
                    ...
                    let nextScene = GameOverScene6(size: self.scene!.size)
            ...
            player.runAction(sequence) { [unowned self] in
                ...
                let nextScene = SuccessScene6(size: self.scene!.size)
                nextScene.scaleMode = SKSceneScaleMode.AspectFill
