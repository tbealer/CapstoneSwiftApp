...
//        addChild(wallIndicator)
        ...
        let lvlindicator = SKSpriteNode(imageNamed: "thereallvl5btn")
    ...
    func loadLevel() {
        if let levelPath = NSBundle.mainBundle().pathForResource("level5", ofType: "txt") {
        ...
        if node.name == "shark" {
            ...
            if (bomb2 >= 1) {
                ...
                player.runAction(sequence) { [unowned self] in
                    ...
                    let nextScene = GameOverScene5(size: self.scene!.size)
            ...
            func updatedefaults () {
                ...
                let defaults = NSUserDefaults.standardUserDefaults()
                defaults.setInteger(1, forKey: "comp6")
                defaults.setInteger(bomb1, forKey: "bombs")
                
                let this = defaults.integerForKey("comp6")
            ...
            player.runAction(sequence) { [unowned self] in
                ...
                let nextScene = SuccessScene5(size: self.scene!.size)
                nextScene.scaleMode = SKSceneScaleMode.AspectFill
