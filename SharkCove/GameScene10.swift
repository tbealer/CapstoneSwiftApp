...
class GameScene10: SKScene, SKPhysicsContactDelegate {
    ...
    override func didMoveToView(view: SKView) {
        ...
        let lvlindicator = SKSpriteNode(imageNamed: "thereallvl10btn")
    ...
    func loadLevel() {
        if let levelPath = NSBundle.mainBundle().pathForResource("level10", ofType: "txt") {
        ...
        if node.name == "shark" {
            ...
            if (bomb2 >= 1) {
                ...
                player.runAction(sequence) { [unowned self] in
                    ...
                    let nextScene = GameOverScene10(size: self.scene!.size)
            ...
            player.runAction(sequence) { [unowned self] in
                ...
                let nextScene = SuccessScene10(size: self.scene!.size)
                nextScene.scaleMode = SKSceneScaleMode.AspectFill
