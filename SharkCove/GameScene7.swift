...
//        addChild(wallIndicator)
        ...
        let lvlindicator = SKSpriteNode(imageNamed: "thereallvl7btn")
    ...
    func loadLevel() {
        if let levelPath = NSBundle.mainBundle().pathForResource("level7", ofType: "txt") {
        ...
        if node.name == "shark" {
            ...
            if (bomb2 >= 1) {
                ...
                player.runAction(sequence) { [unowned self] in
                    ...
                    let nextScene = GameOverScene7(size: self.scene!.size)
            ...
            func updatedefaults () {
                ...
                let defaults = NSUserDefaults.standardUserDefaults()
                defaults.setInteger(1, forKey: "comp8")
                defaults.setInteger(bomb1, forKey: "bombs")
                
                let this = defaults.integerForKey("comp8")
            ...
            player.runAction(sequence) { [unowned self] in
                ...
                let nextScene = SuccessScene7(size: self.scene!.size)
                nextScene.scaleMode = SKSceneScaleMode.AspectFill
